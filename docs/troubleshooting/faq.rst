...
#. `RoboRacer Github Repository <https://github.com/f1tenth>`_: Official RoboRacer github repository.
...
2. Confirm that you built the servo_out firmware for the VESC as shown in the `VESC firmware update instructions <firmware/firmware_vesc.html#updating-the-firmware-on-the-vesc>`_.
...
Second check that you have properly updated the parameters for system identification in the FOC tab. See the image in the `motor parameter detection section <firmware/firmware_vesc.html#detecting-and-calculating-motor-parameters>`_ of the VESC tuning/setup instructions.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
The base plate is a simple laser cut piece. The CAD files are provided in this `shared Google Drive folder <https://drive.google.com/drive/u/1/folders/1o3jRww0UwfmjTBDACD8qu7SDabRzpr5g>`_.
...
In these cases, it is possible and recommended to use a bluetooth gamepad like the Gamepad for PlayStation 4. You'll need a usb bluetooth dongle too. Instructions on how to connect the gamepad via bluetooth can be found in this `Bluetooth pairing video <https://youtu.be/v_neNpfQ38Q?t=386>`_.
...
If this fails to work then there is a strong chance that you need to install the ttyACM module. For a convenient installer, see the `ttyACM module installer <https://github.com/jetsonhacks/installACMModule>`_.